...
local function isVisible(part)
    ...
    raycastParams.FilterDescendantsInstances = {dwLocalPlayer.Character, dwCamera}
    ...
    if settings.Aiming then
        ...
        for i,v in next, dwEntities:GetChildren() do 
            ...
            v.Character:FindFirstChild("Humanoid").Health > 0 then
                ...
                if (settings.Aimbot_TeamCheck and v.Team ~= dwLocalPlayer.Team) or not settings.Aimbot_TeamCheck then
    local char = v.Character
    local char_part_pos, is_onscreen = dwCamera:WorldToViewportPoint(char[settings.Aimbot_AimPart].Position)

    if is_onscreen and isVisible(char[settings.Aimbot_AimPart]) then
...
local currentLook = dwCamera.CFrame.LookVector
local desiredLook = (targetPos - camPos).Unit
local smoothFactor = 0.15 -- ajusta a suavidade
local newLook = currentLook:Lerp(desiredLook, smoothFactor)
dwCamera.CFrame = CFrame.new(camPos, camPos + newLook)
